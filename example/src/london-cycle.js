...
function getCycleStations() {
  return fetch('https://tfl.gov.uk/tfl/syndication/feeds/cycle-hire/livecyclehireupdates.xml') // eslint-disable-line
    .then(res => res.text())
    .then(data =>
...
export default class LondonCycle extends Component {
  ...
  _onToggleHover(cursor, { map }) {
    map.getCanvas().style.cursor = cursor;  // eslint-disable-line
  }
